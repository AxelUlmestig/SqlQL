...
import           SQL.AST
import qualified SQL.Token                          as T
...
whiteSpace = skipMany (satisfy isSpace)
  where
    isSpace (T.WhiteSpace _) = True
    isSpace T.Newline        = True
    isSpace (T.Comment _)    = True
    isSpace _                = False

lexeme :: Parser a -> Parser a
lexeme = L.lexeme whiteSpace

selectT :: Parser T.Token
selectT = satisfy (== T.Select)

dotT :: Parser T.Token
dotT = satisfy (== T.Dot)

commaT :: Parser T.Token
commaT = satisfy (== T.Comma)

asT :: Parser T.Token
asT = satisfy (== T.As)

starT :: Parser T.Token
starT = satisfy (== T.Star)

-- is there a prettier way to do this?
properNounT :: Parser Text
properNounT = extractPN <$> satisfy isPN
  where
    isPN :: T.Token -> Bool
    isPN (T.ProperNoun _) = True
    isPN _                = False

    extractPN (T.ProperNoun pn) = pn
...
selectColumnsP = lexeme $ NE.sepBy1 selectColumnP (lexeme commaT)
...
selectColumnP = lexeme $
  ...
  where
    starP       = SelectAll <$ starT
    tableStarP  = SelectAllTable <$> properNounT <* whiteSpace <* dotT <* whiteSpace <* starT

-- TODO: what about schemas?
alias :: Parser a -> Parser (Aliased a)
alias p = lexeme $ do
  x <- lexeme p
  a <- optional (lexeme asT *> properNounT)
  return (Aliased x a)

expressionP :: Parser Expression
expressionP = ExpColumn <$> columnP

columnP :: Parser Column
columnP = do
  x <- properNounT
  y <- optional (dotT *> properNounT)
